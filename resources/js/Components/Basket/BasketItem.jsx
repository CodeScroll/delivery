...
export default function BasketItem({ item }) {
    ...
    const priceValue = item.price ?? item.estimated_price;

    const isEstimated = !item.price && item.estimated_price;
                ...
                <div className="mt-2 flex items-center justify-between">
                    ...
                    {priceValue && (
                        <div className="flex flex-col">
                            <span>{isEstimated ? `~ ${fmt(priceValue * item.quantity)}` : fmt(priceValue * item.quantity)}</span>
                        </div>
                    )}
                </div>
            </div>
